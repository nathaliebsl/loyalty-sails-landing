        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-6 max-w-5xl mx-auto">
          {teamMembers.map((member, index) => {
            ...
            return (
              <div
                key={index}
                className="group bg-card rounded-lg border border-border p-8 text-center hover:border-primary/50 transition-all duration-500"
              >
                ...
                {member.status && (
                  <p className="text-xs italic uppercase tracking-wider text-tertiary">
                    {member.status}
                  </p>
                )}
              </div>
